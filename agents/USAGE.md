...
| `"bagofholding_hdf"` | HDF5 file(s) via `bagofholding` | `root` | optional `version_validator`, `prefix_length` (multiplex keys sharing an N-char digest prefix into one shared `.h5` file instead of one file per key; default `2`, `0` for one file per key, `None` infers from existing files; validated against existing files in `root` at construction — migrate a live storage with `refix(n)`, repair a mixed root with the `consolidate(root, n)` classmethod) |

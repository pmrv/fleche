...
`bagofholding_hdf` work the same way, and `template = "sql"` pairs
`cloudpickle` values (`root`) with SQL call storage (`url`). The same dicts
work in Python via `cache({"template": "cloudpickle", "root": ".cache"})`.

...
| File | Role |
|------|------|
| `wrapper.py` | `@fleche()` decorator; re-exports `Ignored`/`Required` from `call.py`; `process_ignore_required_args` merges explicit `ignore=`/`require=` args into a `FunctionProfile` via `dataclasses.replace`; attaches `.call/.digest/.load/.contains/.query/.rerun/.bind` helpers (also under `.fleche.*`). Helper attachment is data-driven via `_HelperSpec`/`_HelpersCtx` — `_PRE_WRAPPER_SPECS` (helpers seeding `ctx.get_call`/`ctx.digest_func`) build first, then `make_wrapper` produces `wrapper`, then `_POST_WRAPPER_SPECS` (helpers needing `ctx.wrapper`, e.g. `rerun`/`bind`) build. |
| `digest.py` | `Digest(str)` (with `.expand`/`.shrink`); `digest()` (SHA256 hex); `Hook`, `add_hook`, `get_hooks`, `load_entry_points`; `Indigestible`; `DIGEST_LENGTH=64`. Built-in types are inline `match` cases in `_digest_bytes`: `str`/`bytes`/`int`/`bool`/`None`; `Number` (covers `float`, `complex`, `Decimal`, `Fraction`, and `np.integer`/`np.floating` via `numbers.Real` registration); explicit `np.bool_` arm (delegates to `digest(bool(value))` because `np.bool_ ∉ Number` since numpy 1.20) and `np.ndarray` (dtype + shape + `tobytes`); `types.FunctionType`/`CodeType`; `staticmethod`/`classmethod`/`property`; `datetime.{timezone,timedelta,datetime,date,time}`; `dataclasses` and `attrs` instances (via flat field-dict, so the two layouts hash identically); `types.ModuleType` (digests `__all__` if defined else `dir()`); `Mapping`; `Iterable`. User types opt in by defining `__digest__` on the class — checked before the built-in cases so dict/list subclasses can override (negative answers are cached in `_TYPES_WITHOUT_DIGEST` so the hot path is one set lookup). The `Hook` registry (`add_hook`) and entry-point plugin loading (`[project.entry-points."fleche"]`, key `digest`) are for *third-party* types you can't modify; the hot path skips `get_hooks()` entirely when both registries are empty. Entry points are lazy-loaded on first `Indigestible` and retried once. NaN floats are packed via `struct.pack` (sign-preserving) instead of going through `hash()`; complex NaNs use `<dd`. |
...
- `unit/` — one subdirectory per module under test (`caches/`, `call/`, `config/`, `digest/`, `fleche/`, `metadata/`, `storage/`); filenames mirror the feature being tested. Some non-obvious starting points: `digest/test_entry_points.py` (third-party hook loading), `digest/test_attrs.py` (attrs-class digest path), `call/test_partial_binding.py` + `call/test_matches.py` (`QueryCall` semantics), `call/test_dehydrate.py` (`Call`↔`DigestedCall`↔`LazyCall` round-trips), `storage/test_operation_context.py` (mixin chaining), `storage/test_transform.py`, `storage/test_overwrite.py`, `storage/test_short_digest.py`, `storage/test_thread_safe.py`, `storage/test_secure_storage.py`, `storage/test_optional_deps.py`, `fleche/test_processpool.py`, `fleche/test_futures.py`, `fleche/test_workdir.py`. Two top-level files: `test_cache_sticky.py` (sticky `cache()` context semantics) and `test_pickle.py` (pickling caches/wrappers).

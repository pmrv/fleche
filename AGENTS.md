...
- `unit/` — one subdirectory per module under test (`caches/`, `call/`, `config/`, `digest/`, `fleche/`, `metadata/`, `storage/`); filenames mirror the feature being tested, so for most questions a single `ls` of the relevant subdir is enough. Non-obvious landing spots: `digest/test_entry_points.py` (third-party hook loading), `digest/test_attrs.py` (attrs-class digest path); `call/test_partial_binding.py` + `call/test_matches.py` (`QueryCall` semantics), `call/test_dehydrate.py` (`Call`↔`DigestedCall`↔`LazyCall` round-trips); `caches/test_redigest.py`, `caches/test_gc.py`, `caches/test_lazy_call.py`; `storage/test_operation_context.py` (mixin chaining), `storage/test_transform.py`, `storage/test_overwrite.py`, `storage/test_short_digest.py`, `storage/test_thread_safe.py`, `storage/test_secure_storage.py`, `storage/test_optional_deps.py`, `storage/test_destructuring_dataclasses.py` + `storage/test_destructuring_storage.py`, SQL-specific: `storage/test_sql_query.py` (query pushdown), `storage/test_sql_pragmas.py`, `storage/test_sql_url_coercion.py`, `storage/test_sql_digest_types.py`; `fleche/test_processpool.py`, `fleche/test_futures.py`, `fleche/test_workdir.py`, `fleche/test_signature_binding.py` (ignore/require interactions), `fleche/test_rerun.py`. Two top-level files: `test_cache_sticky.py` (sticky `cache()` context semantics) and `test_pickle.py` (pickling caches/wrappers). Optional-dep tests use `pytest.importorskip(...)` — don't wrap imports in `try/except ImportError`. No `[tool.pytest.ini_options]` in `pyproject.toml`; the only pytest config is `conftest.py` (which registers `tests.fixtures` as a plugin).

...
| `query.py` | `QueryIterator` over `LazyCall` — lazy: `take/skip/filter/unique` return new `QueryIterator`s; `results()` returns a plain `Iterator[Any]` (not chainable). Eager (consume the iterator): `only/any/count/empty/sorted/groupby/latest/oldest/evict/table`. `QueryIterator.calls` is a *factory* (zero-arg callable returning a fresh iterable) so the iterator is re-iterable — every `for` / `list()` / consuming method starts a new traversal and reflects current cache state. `.table()` → pandas DataFrame (auto-converts `timestart`/`timestop` to local tz; argument names that clash with built-in/metadata columns are prefixed `a_`). `latest`/`oldest` need `Runtime` metadata. `sorted`/`unique`/`groupby` accept either a callable or an argument-name string (resolved via `_resolve_key` to `c.arguments[name]`). |
...
5. Special cases: `None` return → not cached (warning). `Indigestable` arg → call runs uncached. Missing `Required` kwargs → call runs uncached.

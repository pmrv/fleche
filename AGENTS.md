...
**Public API (`fleche.__init__.__all__`):** decorator + state helpers (`fleche`, `cache`, `meta`, `tags`, `project`, `BoundWrapper`), arg markers (`Ignored`, `Required`), plus two non-obvious helpers:
- `D(value)`: returns a `Digest`. Non-empty hex strings up to 64 chars pass through verbatim (so you can use a digest prefix as a lookup key); anything else is digested. `Digest` args to a wrapped function are auto-expanded to their cached values before hashing.
- `wrap_executor(executor)`: monkey-patches `submit` so already-cached fleche calls bypass the executor and misses are auto-`bind()`'d.
...
| `digest.py` | `Digest(str)` (with `.expand`/`.shrink`); `digest()` (SHA256 hex); `Hook`, `add_hook`, `get_hooks`, `load_entry_points`; `Unhashable`; `DIGEST_LENGTH=64`. Built-in types are dispatched via inline `match` cases in `_digest` (see source); the `Hook` registry and `[project.entry-points."fleche.digest"]` plugin loading are for *third-party* types — entry points are lazy-loaded on first `Unhashable` and retried once (`digest()` wraps `_digest`). User types can also implement `__digest__` for direct override. |
| `call.py` | `Call` dataclass (`from_call`, `to_lookup_key`, `.stash(values)→DigestedCall` writes args+result to value storage / `.digest()→DigestedCall` digests without saving); `DigestedCall` (storage representation: arguments and result are `Digest` keys; `.fetch(cache)→LazyCall`); `LazyCall` (private `_arguments`/`_result`, deferred deser via `_cache`; `LazyArguments` Mapping resolves digests on access — **swallows `KeyError` and returns the raw `Digest`** if the value is missing from storage); `QueryCall` (wildcard match via `matches`, `partial=True` binding); `bind()` wrapper over `inspect.Signature.bind[_partial]`; `AnyCall = Call \| LazyCall`. All four call types digest identically when their fields agree. Name/module/version come from `pyiron_snippets.versions.VersionInfo`; `func.__version__` overrides the module-level version. |
| `caches.py` | `BaseCache`, `Cache`, `CacheStack`, `ReadOnlyCache`, `FilteredCache`, `RefreshingCache`, `SizeLimitedMixin`/`SizeLimitedCache`; `Rejected` exception. On `BaseCache`: `transfer`, `readonly`, `push`, `filter`, `table`, `query(template_or_None, **kwargs)` (template *or* kwargs, not both). On `Cache` only: `redigest` (re-saves any call whose `to_lookup_key()` no longer matches its stored key — used after a hash-function change). `Cache.contains` short-circuits to `self.calls.contains(key)` (no value deser); `BaseCache.contains` falls back to load+catch. `CacheStack` saves to bottom (`stack[0]`), loads top-down and back-fills hits to bottom; cannot nest. |
...
- `memory.py`, `void.py`, `pickle_file.py` (+`PickleFileBackend.with_pickle`/`with_cloudpickle`/`with_dill`; `compress_all`/`decompress_all` migration helpers; gzip auto-detected by `\x1f\x8b` magic on read), `bagofholding_file.py`, `sql.py` — concrete backends (each exposes `Value*` and/or `Call*` classes; `sql.py` only has `Sql` for calls). `Sql` accepts `url=None` (defaults to `sqlite:///:memory:`) or a bare path (coerced to `sqlite:///<abs>`); SQLite enables `PRAGMA foreign_keys=ON` per connection. Schema: `calls` (key, name, module, version-as-JSON, code_digest, result-digest), `arguments` (call_key, position, name, value-as-digest), `metadata` (call_key, name, JSON `data`). `Sql.query` always pushes call/argument filters down to SQL (arguments compared as `digest(value)` strings via `JOIN`s on the `arguments` table); metadata filters are pushed down only when *every* filter value is a simple scalar (`str`/`bool`/`int`/`float`) via JSON-extract `as_*` casts — any `None` or complex (e.g. list) value falls back to client-side metadata filtering after the SQL fetch (call/argument filters still run server-side).
- `destructuring.py` — `DestructuringMixin` for recursive value splitting + `Digested` ABC with three concrete markers: `DigestedIterable` (lists/tuples), `DigestedDict`, and `DigestedDataclass` (any non-NamedTuple dataclass — `mend` reconstructs via `object.__new__` + `__setattr__`, bypassing `__init__`/`__post_init__`, so `init=False`/`InitVar` fields round-trip). All preserve digest equivalence via `__digest__`. `DestructuringMixin` is a `ValueStorage` subclass — operates at the `save`/`load` layer, not `put`/`get`; compose **above** `ValueMixin` in the MRO. `remaining_depth` (default `0`) controls how deep structures are split across keys. `count_reuses()` reports how often each key is referenced as a sub-component (useful for GC-style audits). NamedTuples are deliberately **not** destructured (`_is_trojan_tuple` guard).
...
Runtime hard deps: `numpy`, `pandas (>=2.2,<4)`, `pyiron_snippets` (used for `versions.VersionInfo` in `call.py` and `import_alarm.ImportAlarm` everywhere optional deps are gated).

Optional dep extras: `cloudpickle`, `dill`, `sqlalchemy`, `bagofholding`, `executorlib`, `docs`, `tests` (the `tests` extra already pulls in cloudpickle/dill/sqlalchemy/bagofholding). Optional deps are gated via `ImportAlarm` — importing a backend without its extra installed raises at construction, not at module import.

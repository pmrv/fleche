...
* :class:`~fleche.storage.DillFile`: Stores data as files on the filesystem, using ``dill`` for serialization. It handles more complex Python objects than standard ``pickle`` and can handle objects that ``cloudpickle`` might not. It takes a ``root`` argument, which is the path to the directory where the files will be stored.

...
def test_in_flight_covers_cache_write_gap():
    ...
    A RaceLatch holds the done-callback at the start of cache.save.  While it
    is paused, a second compute() call is made; it should find the key in
    _in_flight and return without incrementing call_count.  Both calls use the
    same key, so the cache count alone can't distinguish the fix from the bug;
    instead we assert on call metadata -- the function is invoked exactly once
    (call_count) and cache.save runs exactly once (save_count).
    """
    latch = RaceLatch()
    save_count = [0]
    call_count = [0]
    ...
    class SlowSaveCache(Cache):
        def save(self, call):
            # Pause at the start of save so the _in_flight window stays open
            # for the second compute() call below.
            latch.pause()
            save_count[0] += 1
    ...
    try:
        c = SlowSaveCache(ValueMemory({}), CallMemory({}))
        ...
        with cache(c):
            ...
            # Wait until the callback has entered save (meaning _in_flight[key]
            # is now set but cache.save has not yet run).

...
def test_readonly_cache_load():
    ...
    c.load("key")
    mock_cache.load.assert_called_once_with("key", delayed=False)
...
def test_cache_stack_load_hit():
    ...
    c1.load.assert_called_once_with("key", delayed=False)
    c2.load.assert_called_once_with("key", delayed=False)
    assert result == call
